...
import { ConstraintEngine, Severity } from "./index.js";
...
function cmdCheck(opts: Record<string, string>): void {
  ...
  const paddedValues: Record<string, number> = {};
...
function cmdBench(opts: Record<string, string>): void {
  const presetName = opts.preset;
  if (!presetName) {
