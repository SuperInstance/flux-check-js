...
export function severityFromMask(mask: number): Severity {
 ...
 */
export function checkOne(value: number, lo: number, hi: number): number {
 ...
 */
export function errorMask(violations: Uint8Array | number[]): number {
  let mask = 0;
