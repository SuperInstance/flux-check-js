...
export interface DriftInfo {
  drifting: boolean;
  perSensor: Record<string, { direction: string; rate: number }>;
...
export class DriftDetector {
  ...
  private sensors: Map<string, SensorWindow> = new Map();
  private bounds: Map<string, { lo: number; hi: number }> = new Map();
  private tickCount = 0;
  ...
  detectDrift(): DriftInfo {
    const perSensor: Record<string, { direction: string; rate: number }> = {};
    const timeToViolation: Record<string, number | null> = {};
   ...
   */
  forecast(nAhead: number = 5): Record<string, number>[] {
 ...
 */
function estimateTimeToViolation(
  currentValue: number,
  rate: number,
  lo: number,
  hi: number
  ...
  if (rate > 0) {
    ...
    if (currentValue >= hi) return 0; // already violated
    if (currentValue < hi) {
    ...
    if (currentValue <= lo) return 0; // already violated
    if (currentValue > lo) {
