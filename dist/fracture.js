...
export class DependencyGraph {
    ...
    constraintNames;
    dimensionNames;
    constructor(adjacency, constraintNames, dimensionNames) {
    ...
    static fromMasks(masks, constraintNames, dimensionNames) {
    ...
    involves(constraintIdx, dimensionIdx) {
        return this.adjacency[constraintIdx]?.[dimensionIdx] === 1;
    }
    constraintDims(constraintIdx) {
        const dims = [];
    ...
    dimConstraints(dimensionIdx) {
        const constraints = [];
        for (let i = 0; i < this.nConstraints; i++) {
            if (this.adjacency[i]?.[dimensionIdx])
                constraints.push(i);
        }
        return constraints;
    }
 ...
 */
export function fracture(graph) {
 ...
 */
export function coalesce(blockMasks) {
    let result = 0;
 ...
 */
export function coalesceArrays(blockArrays) {
 ...
 */
export function fractureFromConstraints(constraints) {
